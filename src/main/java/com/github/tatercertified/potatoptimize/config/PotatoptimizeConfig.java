...
public class PotatoptimizeConfig {
    ...
    private void applyModOverrides() {
        for (ModContainer container : FabricLoader.getInstance().getAllMods()) {
            ...
            if (meta.containsCustomValue(JSON_KEY_MOD_OPTIONS)) {
                CustomValue overrides = meta.getCustomValue(JSON_KEY_MOD_OPTIONS);

                if (overrides.getType() != CustomValue.CvType.OBJECT) {
                    LOGGER.warn("Mod '{}' contains invalid Potatoptimize option overrides, ignoring", meta.getId());
                    continue;
                }
    ...
    private static void writeDefaultConfig(File file) throws IOException {
        ...
        try (Writer writer = new FileWriter(file)) {
            ...
            writer.write("# This file exists for enabling and disabling optimizations via mixins.\n");
            writer.write("# Before configuring anything, take a backup of the worlds that will be opened.\n");
            writer.write("#\n");
            writer.write("# You can find information on editing this file and all the available options here:\n");
            writer.write("# https://github.com/Tater-Certified/Potatoptimize/wiki/Configuration-File\n");
            writer.write("#\n");
